...
// FIX: importação única — forecasts_pt.js e personalForecast_pt.js são o mesmo arquivo.
// A versão anterior importava ambos causando erro de módulo duplicado.
...
// FIX: _cardCounter declarado no topo do módulo (era declarado no meio do arquivo,
// após buildCard que o usa, causando ReferenceError em alguns engines)
var _cardCounter = 0;
var _activeTab = 'identidade';
var _globalEventsInit = false; // guarda contra vazamento de listener em recálculos
...
function attachTabEvents() {
  ...
  function switchTab(tab) {
    ...
    if (btn) {
      ...
      if (window.innerWidth < 820) btn.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
    }
    var panel = document.getElementById('panel-' + tab);
    if (panel) { panel.style.display = 'block'; panel.classList.add('active'); }
  }

  // FIX: listener de clique nas abas — re-anexado em cada renderResults()
  ...
  // FIX: listeners globais (teclado, touch) registrados apenas uma vez
  ...
  // Tabs internas dos cards (seções dentro de cada card)
...
function renderIdentidade(data) {
  ...
  html += buildLetterTile(
    data.cornerstone.letter, data.cornerstone.value.final,
    t('labelCornerstone'), t('labelCornerstoneSub'),
    CORNERSTONE_TEXTS[data.cornerstone.letter] || ''
  );
  html += buildLetterTile(
    data.capstone.letter, data.capstone.value.final,
    t('labelCapstone'), t('labelCapstoneSub'),
    CAPSTONE_TEXTS[data.capstone.letter] || ''
  );
  html += buildLetterTile(
    data.firstVowel.letter, data.firstVowel.value.final,
    t('labelFirstVowel'), t('labelFirstVowelSub'),
...
function renderJornada(data) {
  ...
  data.pinnacles.forEach(function (p) {
...
function buildLetterTile(letter, value, label, sub, interp) {
  ...
  var interpHtml = interp ? '<div class="s-tile-interp">' + escH(interp) + '</div>' : '';
...
function buildComplementarTile(numResult, label, sub, interp) {
  ...
  var isMaster = MASTERS[n]                           ? ' master' : '';
  var isKarmic = (numResult && KARMIC[numResult.pre]) ? ' karmic' : '';
  var interpHtml = interp ? '<div class="complementar-interp">' + escH(interp) + '</div>' : '';
...
function buildPlanesBlock(planes) {
  ...
  var attentionKeys   = lowestKeys.filter(function (k) { return counts[k] === 0 || (maxVal - counts[k]) >= 3; });
  ...
  plDefs.forEach(function (p) {
    ...
    var cellCls = 'el-cell' + (isDom ? ' dominant' : '') + (isLow ? ' el-attention' : '');
    ...
    if (isLow && !isDom) html += '<div class="el-attention-tag">atenção</div>';
    html += '<div class="el-bar-wrap"><div class="el-bar" style="width:' + (maxVal ? Math.round(cnt / maxVal * 100) : 0) + '%"></div></div>';
    html += '</div>';
  });
  ...
  var interpText = '';
  if (isBalanced)               interpText = PLANE_SPECIAL.equilibrio       || '';
  else if (dominantKeys.length) interpText = PLANE_DOMINANT[dominantKeys[0]] || '';
...
function buildCycleReading(cycles, age) {
  var activeCyc = null, activeIdx = -1;
  cycles.forEach(function (cyc, i) {
    if (age >= cyc.start && (cyc.end === null || age < cyc.end)) { activeCyc = cyc; activeIdx = i; }
  });
  ...
  var posLabel = activeIdx === 0 ? 'Ciclo Formativo' : activeIdx === 1 ? 'Ciclo de Consolidação' : 'Ciclo de Conclusão';

  var html = '<div class="cycle-reading mb20">';
  html += '<div class="cycle-reading-header">';
  html += '<span class="cycle-reading-num' + (!!MASTERS[activeCyc.num] ? ' is-master' : '') + '">' + activeCyc.num + '</span>';
...
function buildPinnacleReading(pinnacles, age) {
  ...
  var phaseDesc = ['Estabelecimento · Primeiras provas', 'Consolidação · Aprofundamento', 'Expressão máxima · Contribuição', 'Síntese · Legado'];

  var activePinn = pinnacles[activeIdx];
  var textMap    = PINNACLE_TEXTS[posKeys[activeIdx]];
  var activeText = textMap ? (textMap[activePinn.num] || '') : '';
  if (!activeText) return '';

  var ageRange = activePinn.end ? (activePinn.start + '\u2013' + activePinn.end + ' anos') : (activePinn.start + '+ anos');
  var html = '<div class="pinnacle-reading mb20">';
  html += '<div class="pinnacle-reading-header">';
  html += '<span class="pinnacle-reading-num' + (MASTERS[activePinn.num] ? ' is-master' : '') + '">' + activePinn.num + '</span>';
  ...
  html += '<div class="essence-body"><p class="pinnacle-reading-text">' + escH(activeText) + '</p></div></details>';
  html += '</div>';
  ...
  if (hasOthers) {
    ...
    pinnacles.forEach(function (p, i) {
      ...
      var pText = PINNACLE_TEXTS[posKeys[i]] ? (PINNACLE_TEXTS[posKeys[i]][p.num] || '') : '';
      if (!pText) return;
      var ages = p.end ? (p.start + '\u2013' + p.end + ' anos') : (p.start + '+ anos');
      ...
      html += '</div><p class="pinnacle-other-text">' + escH(pText) + '</p></div>';
    });
    html += '</div></details>';
  }
  return html;
...
function buildChallengeReading(challenges, pinnacles, age) {
  ...
  var html = '<div class="challenge-reading mb20">';
  html += '<div class="pinnacle-reading-header' + (isZero ? ' challenge-zero-header' : '') + '">';
  html += '<span class="pinnacle-reading-num' + (isZero ? ' challenge-zero-num' : '') + '">' + activeCh.num + '</span>';
  html += '<div class="pinnacle-reading-meta">';
  html += '<span class="pinnacle-reading-pos">' + escH(activeLabel) + '</span>';
  html += '<span class="pinnacle-reading-phase">' + (isZero ? 'O desafio da escolha total' : 'Padrão de resistência desta fase') + '</span>';
  ...
  if (hasOthers) {
    ...
    challenges.forEach(function (ch, i) {
      ...
      html += '<div class="pinnacle-other-item">';
      html += '<div class="pinnacle-other-header"><span class="pinnacle-other-num">' + ch.num + '</span><span class="pinnacle-other-label">' + escH(lbl) + '</span></div>';
      html += '<p class="pinnacle-other-text">' + escH(chText) + '</p></div>';
    });
    html += '</div></details>';
  }
  return html;
...
function renderRadar(data) {
  ...
  var elDefs = [{ lbl: t('labelFogo') }, { lbl: t('labelAr') }, { lbl: t('labelTerra') }, { lbl: t('labelAgua') }];
...
function renderPadroes(data) {
  ...
  var tbl  = data.pythTable;
  var maxT = Math.max.apply(null, [1, 2, 3, 4, 5, 6, 7, 8, 9].map(function (n) { return tbl[n]; }));
...
function buildKarmicLessonsBlock(karmicLessons) {
  ...
  if (!karmicLessons.length) {
    html += '<div class="karmic-lessons mb20"><div class="karmic-lessons-nums"><span class="kl-none">' + escH('Nenhuma — todas as vibrações estão presentes no nome.') + '</span></div></div>';
    return html;
  }
  html += '<div class="karmic-lessons-block mb20">';
  if (karmicLessons.length >= 3) html += '<div class="kl-multiple-note">' + escH(KARMIC_MULTIPLE_NOTE) + '</div>';
  html += '<div class="kl-list">';
  karmicLessons.forEach(function (n) {
    var text = KARMIC_LESSON_TEXTS[n] || '';
    html += '<details class="kl-item"><summary class="kl-summary">';
    ...
    html += '</summary>';
    if (text) html += '<div class="kl-body">' + escH(text) + '</div>';
    html += '</details>';
  });
  html += '</div></div>';
  return html;
...
function renderComplementar(data) {
  ...
  html += '<div class="complementar-block mb20"><div class="complementar-context">' + escH(BALANCE_CONTEXT) + '</div><div class="tiles-grid">';
  html += buildComplementarTile(data.balance,     t('labelBalance'),      t('labelBalanceSub'),      BALANCE_TEXTS[balanceNum]          || '');
  html += '</div></div>';

  html += '<div class="complementar-block mb20"><div class="complementar-context">' + escH(RATIONAL_CONTEXT) + '</div><div class="tiles-grid">';
  html += buildComplementarTile(data.rational,    t('labelRational'),     t('labelRationalSub'),     RATIONAL_TEXTS[rationalNum]        || '');
  html += '</div></div>';

  html += '<div class="complementar-block mb20"><div class="complementar-context">' + escH(SUBCONSCIOUS_CONTEXT) + '</div><div class="tiles-grid">';
...
function renderPrevisoes(data) {
  ...
  var todayStr = dd + '/' + mm + '/' + String(curYr);
  ...
  projection.forEach(function (p) {
    ...
    if (isCur)       html += '<div class="proj-current-tag">' + escH(t('labelCurrentYear')) + '</div>';
    else if (p.isMaster) html += '<div class="proj-current-tag">\u2736</div>';
    html += '</div>';
  });
  html += '</div>';
...
function buildPersonalYearReading(yearNum) {
  ...
  // FIX: forecasts_pt.js usa string com \n\n em vez de objeto {theme,areas,guidance}
  // Detectar formato e adaptar
  var bodyText = '';
  if (typeof entry === 'string') {
    bodyText = entry;
  } else {
    bodyText = [theme, areas, guidance].filter(Boolean).join('\n\n');
  }

  var html = '<div class="forecast-reading mb20">';
  ...
  if (bodyText) html += '<p class="forecast-p">' + escH(bodyText) + '</p>';
...
function buildPersonalMonthReading(monthNum, todayMonthIdx, mNames) {
  ...
  var html = '<div class="forecast-month-reading mb20">';
...
function buildPersonalDayNote(dayNum) {
  var html = '<div class="forecast-day-note mb20">';
...
function buildCard(numResult, role, badgeClass, calType) {
  ...
  html += '<div class="num-badge ' + badgeClass + '">' + n + '</div>';
...
function buildCardBody(n, pre, isMaster, isKarmic, karmicKey, calType) {
  if (isKarmic && karmicKey) return buildKarmicBody(karmicKey);
  if (isMaster)              return buildMasterBody(n, calType);
...
function buildNormalBody(n, calType) {
  ...
  var formulaHtml = formulaText !== formulaKey
    ? '<div class="formula-line"><span class="formula-tag">' + t('formulaLabel') + '</span> ' + escH(formulaText) + '</div>'
    : '';
...
function buildMasterBody(n, calType) {
  ...
  if (!texts) return '';
  var sections = [
    { key: 'motivation', label: t('tabMotivation'), content: texts.motivation },
    { key: 'expression', label: t('tabExpression'), content: texts.expression },
    { key: 'shadow',     label: t('tabShadow'),     content: texts.shadow, isShadow: true }
  ];
...
function buildKarmicBody(karmicKey) {
  ...
  if (!texts) return '';
  var sections = [
    { key: 'pattern',       label: t('tabPattern'),       content: texts.pattern       },
    { key: 'manifestation', label: t('tabManifestation'), content: texts.manifestation },
    { key: 'integration',   label: t('tabIntegration'),   content: texts.integration   }
  ];
